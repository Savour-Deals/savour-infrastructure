...
export default async function main(event) {
  console.log(event);
  const bNum = event.To.replace("%2B", "+");
  const uNum = event.From.replace("%2B", "+");
  const eventMsg = event.Body;
  let resp = '';
  let respBody = '';
...
async function subscribeUser(uNum, bNum){
  ...
  let businessInfo = await dynamoDbLib.call("query", bParams);
  if (businessInfo.Items.length > 0) {
    // business exists, continue
    businessInfo = businessInfo.Items[0];
    const userInfo = await dynamoDbLib.call("query", uParams);
    if (userInfo.Items.length > 0) {
      // subscriber exists in DB, continue
      const subDict = userInfo.Items[0].subscription_dict;
